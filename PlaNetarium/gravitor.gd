...
func add_child_from_elements(
	...
	var child := Gravitor.new(initial_state[0], initial_state[1], mu_)
...
static func make_root_gravitor(mu_ : float) -> Gravitor:
	return Gravitor.new(DoubleVector3.ZERO(), DoubleVector3.ZERO(), mu_)
...
func _init(pos_0_ : DoubleVector3, vel_0_ : DoubleVector3, mu_ : float) -> void:
	pos_0 = pos_0_
	vel_0 = vel_0_
	mu = mu_


## Gravitor tree query: returns a list of state [pos, vel, mu] 'tuples' for every
## gravitor in the tree at the specified time. 
## SHOULD ONLY BE INVOKED ON THE ROOT GRAVITOR.
func all_states_at_time(time : float) -> Array:
	var output := []
	_tree_state_query_kernel(time, output, DoubleVector3.ZERO(), DoubleVector3.ZERO())
	
	# Add ourself, the root:
	output.append([DoubleVector3.ZERO(), DoubleVector3.ZERO(), mu])
	
	return output


# Recursive kernel for the above. Appends to given output array.
func _tree_state_query_kernel(time : float, output : Array, pos : DoubleVector3, vel : DoubleVector3) -> void:
	
	# For every child:
	for child in children:
		# Establish the child's local state, using the Kepler query on our own gravity param
		var local_state := UniversalKepler.query(child.pos_0, child.vel_0, 0.0, time, mu)
